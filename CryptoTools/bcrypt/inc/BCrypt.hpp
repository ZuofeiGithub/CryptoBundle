...
public:
    ...
    static ByteArray generateHash(const ByteArray& data, int workload = 12){
        char salt[BCRYPT_HASHSIZE];
        char hash[BCRYPT_HASHSIZE];
        int ret;
        ret = bcrypt_gensalt(workload, salt);
        if(ret != 0)throw std::runtime_error{"bcrypt: can not generate salt"};
        ret = bcrypt_hashpw(data.data(), salt, hash);
        if(ret != 0)throw std::runtime_error{"bcrypt: can not generate hash"};
        return ByteArray{hash};
    }

    static bool validatePassword(const ByteArray& data,const ByteArray& hash){
        return (bcrypt_checkpw(data.data(), hash.data()) == 0);
    }
};
